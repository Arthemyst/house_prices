...
def remove_outliers_in_columns(columns_for_estimation, df_train_dummies):
    for column in columns_for_estimation:
        Q1 = np.percentile(df_train_dummies[column], 25,
                        method = 'midpoint')
        
        Q3 = np.percentile(df_train_dummies[column], 75,
                        method = 'midpoint')
        IQR = Q3 - Q1
...
def generate_best_model(columns_for_estimation, df_train_dummies, n=50):
    ...
    for run in range(1,n):
        model, model_score, scaler = model_to_estimate_player_value(df_train_dummies, columns_for_estimation)
